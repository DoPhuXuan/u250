...
set project_name proj_bert_12layer_csim
...
set output_dir [file join $root_dir validation outputs bert_base_uncased_seq128]
if {[info exists ::env(BERT_CSIM_OUTPUT)] && $::env(BERT_CSIM_OUTPUT) ne ""} {
    set output_dir [file normalize $::env(BERT_CSIM_OUTPUT)]
}
set report_dir [file join $root_dir reports csim bert_base_uncased_seq128]
if {[info exists ::env(BERT_CSIM_REPORT)] && $::env(BERT_CSIM_REPORT) ne ""} {
    set report_dir [file normalize $::env(BERT_CSIM_REPORT)]
}

foreach required [list \
    [file join $data_dir metadata.json] \
    [file join $data_dir input embedding_output.f32] \
    [file join $data_dir input attention_mask.i32] \
    ...
    if {![file exists $required] || [file size $required] == 0} {
        error "Missing validation input: $required. Run validation/export_hf_bert_fp32.py first."
    }
}
...
set cflags "-std=c++14 -O2 -I[file join $root_dir include]"
set old_dir [pwd]
cd $build_dir
...
add_files -tb -cflags $cflags [file join $root_dir validation tb_bert_12layer_csim.cpp]
...
puts "============================================================"
puts "BERT FP32 full 12-layer C simulation"
puts "Data:    $data_dir"
puts "Outputs: $output_dir"
puts "Reports: $report_dir"
puts "============================================================"
set csim_argv [join [list $data_dir $output_dir] " "]
csim_design -O -clean -argv $csim_argv
...
file copy -force \
    [file join $output_dir csim_layer_outputs.csv] \
    [file join $report_dir csim_layer_outputs.csv]
close_project
cd $old_dir
...
if {$run_compare} {
    ...
    set compare_command [list \
        $python_bin [file join $root_dir validation compare_layer_outputs.py] \
        --data-dir $data_dir \
        --csim-output-dir $output_dir \
        --report-dir $report_dir]
    if {[info exists ::env(CSIM_MAX_RMSE)] && $::env(CSIM_MAX_RMSE) ne ""} {
        lappend compare_command --max-rmse $::env(CSIM_MAX_RMSE)
    }
    if {[info exists ::env(CSIM_MAX_ABS)] && $::env(CSIM_MAX_ABS) ne ""} {
        lappend compare_command --max-abs $::env(CSIM_MAX_ABS)
    }
    if {[info exists ::env(CSIM_MIN_COSINE)] && $::env(CSIM_MIN_COSINE) ne ""} {
        lappend compare_command --min-cosine $::env(CSIM_MIN_COSINE)
    }
    if {[info exists ::env(CSIM_GATE_SCOPE)] && $::env(CSIM_GATE_SCOPE) ne ""} {
        lappend compare_command --gate-scope $::env(CSIM_GATE_SCOPE)
    }
    puts "Running layer-by-layer Hugging Face comparison..."
    if {[catch {exec {*}$compare_command 2>@1} compare_output]} {
        puts stderr $compare_output
        error "C-sim completed, but the accuracy gate failed"
    }
    puts $compare_output
}

puts "C-sim outputs: $output_dir"
puts "Accuracy report: [file join $report_dir accuracy_report.md]"
