...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    volatile unsigned int ready_snapshot = hidden_ready[0];
    (void)ready_snapshot;
...
#pragma HLS ARRAY_RESHAPE variable=v_group cyclic factor=QKV_O_PAR dim=2
    ...
    load_hidden_matrix(hidden_in, hidden);
...
HEAD_GROUP:
    for (int group = 0; group < HEAD_GROUPS; ++group) {
        ...
        write_qkv_group(q_group, k_group, v_group, q_stream, k_stream, v_stream);
    }
}

static void load_hidden_feedback_stream(
    hidden_stream_t &next_hidden_stream,
    float hidden[SEQ_LEN][HIDDEN_SIZE])
{
#pragma HLS INLINE off
FULL_LOAD_FEEDBACK_ROW:
    for (int s = 0; s < SEQ_LEN; ++s) {
    FULL_LOAD_FEEDBACK_PACK:
        for (int p = 0; p < PACKS; ++p) {
#pragma HLS PIPELINE II=1
            float lane[PACK_SIZE];
#pragma HLS ARRAY_PARTITION variable=lane complete dim=1
            unpack_bus16(next_hidden_stream.read(), lane);
        FULL_LOAD_FEEDBACK_LANE:
            for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
                hidden[s][p * PACK_SIZE + i] = lane[i];
            }
...
static void write_attention_residual_stream(
    float hidden[SEQ_LEN][HIDDEN_SIZE],
    hidden_stream_t &attention_residual_stream)
{
#pragma HLS INLINE off
FULL_WRITE_RESIDUAL_ROW:
    for (int s = 0; s < SEQ_LEN; ++s) {
    FULL_WRITE_RESIDUAL_PACK:
        for (int p = 0; p < PACKS; ++p) {
#pragma HLS PIPELINE II=1
            float lane[PACK_SIZE];
#pragma HLS ARRAY_PARTITION variable=lane complete dim=1
        FULL_WRITE_RESIDUAL_LANE:
            for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
                lane[i] = hidden[s][p * PACK_SIZE + i];
            }
            attention_residual_stream.write(pack_bus16(lane));
        }
    }
}

extern "C" void bert_qkv_12layer_kernel(
    ...
    hidden_stream_t &v_stream)
{
#pragma HLS INTERFACE m_axi port=initial_hidden offset=slave bundle=gmem_hidden depth=6144
#pragma HLS INTERFACE m_axi port=initial_hidden_ready offset=slave bundle=gmem_hidden_ready depth=1
#pragma HLS INTERFACE m_axi port=attn_q_w_all offset=slave bundle=gmem_q depth=442368
#pragma HLS INTERFACE m_axi port=attn_q_b_all offset=slave bundle=gmem_q depth=576
#pragma HLS INTERFACE m_axi port=attn_k_w_all offset=slave bundle=gmem_k depth=442368
#pragma HLS INTERFACE m_axi port=attn_k_b_all offset=slave bundle=gmem_k depth=576
...
#pragma HLS INTERFACE s_axilite port=initial_hidden_ready bundle=control
#pragma HLS INTERFACE s_axilite port=attn_q_w_all bundle=control
#pragma HLS INTERFACE s_axilite port=attn_q_b_all bundle=control
#pragma HLS INTERFACE s_axilite port=attn_k_w_all bundle=control
#pragma HLS INTERFACE s_axilite port=attn_k_b_all bundle=control
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    volatile unsigned int ready_snapshot = initial_hidden_ready[0];
    (void)ready_snapshot;
...
FULL_QKV_LAYER:
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        if (layer == 0) {
            load_hidden_matrix(initial_hidden, hidden);
        } else {
            load_hidden_feedback_stream(next_hidden_stream, hidden);
        }

        // Out/Norm consumes the residual before it consumes context.  Emit it
        // first so no finite Q/K/V or context FIFO can create a wait cycle.
        write_attention_residual_stream(hidden, attention_residual_stream);

        const int weight_offset = layer * ATTN_W_PACKS;
        const int bias_offset = layer * ATTN_B_PACKS;
    FULL_QKV_HEAD_GROUP:
        for (int group = 0; group < HEAD_GROUPS; ++group) {
            project_qkv_group(
                hidden,
