...
#define FULL_MODEL_LAYERS 12
#define FFN_W1_WORDS_PER_LAYER ((INTERMEDIATE_SIZE * HIDDEN_SIZE) / PACK_FLOATS)
#define FFN_W2_WORDS_PER_LAYER ((HIDDEN_SIZE * INTERMEDIATE_SIZE) / PACK_FLOATS)
#define FFN_B1_WORDS_PER_LAYER (INTERMEDIATE_SIZE / PACK_FLOATS)
#define FFN_HIDDEN_WORDS_PER_LAYER (HIDDEN_SIZE / PACK_FLOATS)
...
typedef bert_stream_t<packed_float16_t> packed_float_stream_t;

static float gelu_pwl_packed(float x) {
#pragma HLS INLINE
    // Eight constrained quadratic segments approximate the erf-based GELU
    // used by pretrained BERT. Coefficients are selected first and evaluated
    // by one shared datapath with exactly the same arithmetic count as V21:
    // two FP32 multiplications and one addition, x * (a*x + b).
    if (x < -4.0f) {
        return 0.0f;
    }
    if (x >= 4.0f) {
...
static void fp32_initialize_down_bias_residual(
    packed_float_stream_t &ffn_residual_stream,
    packed_float16_t *b2,
    float output_buf[SEQ_LEN][HIDDEN_SIZE])
{
#pragma HLS INLINE off
fp32_down_residual_s:
    for (int s = 0; s < SEQ_LEN; ++s) {
    fp32_down_residual_pack:
        for (int h_pack = 0; h_pack < HIDDEN_PACKS; ++h_pack) {
#pragma HLS PIPELINE II=1
...
#pragma HLS ARRAY_PARTITION variable=residual_lane complete dim=1
            ...
            unpack_packed_float16(ffn_residual_stream.read(), residual_lane);
        fp32_down_residual_lane:
            for (int lane = 0; lane < PACK_FLOATS; ++lane) {
#pragma HLS UNROLL
...
static void v15_ffn_down_tilemajor_projection(
    packed_float_stream_t &gelu_stream,
    packed_float16_t *w2,
    float output_buf[SEQ_LEN][HIDDEN_SIZE])
{
...
static void fp32_layernorm_and_store(
    float output_buf[SEQ_LEN][HIDDEN_SIZE],
...
#pragma HLS ARRAY_RESHAPE variable=gamma_cache cyclic factor=16 dim=1
#pragma HLS ARRAY_RESHAPE variable=beta_cache cyclic factor=16 dim=1
...
#pragma HLS UNROLL
    ...
    fp32_norm_reduce_pack:
        for (int h_pack = 0; h_pack < HIDDEN_PACKS; ++h_pack) {
#pragma HLS PIPELINE II=1
        fp32_norm_reduce_lane:
            for (int lane = 0; lane < PACK_FLOATS; ++lane) {
#pragma HLS UNROLL
                const float value =
                    output_buf[s][h_pack * PACK_FLOATS + lane];
    ...
    fp32_norm_write_pack:
        for (int h_pack = 0; h_pack < HIDDEN_PACKS; ++h_pack) {
#pragma HLS PIPELINE II=1
            float output_lane[PACK_FLOATS];
#pragma HLS ARRAY_PARTITION variable=output_lane complete dim=1
        fp32_norm_write_lane:
            for (int lane = 0; lane < PACK_FLOATS; ++lane) {
#pragma HLS UNROLL
                const int h = h_pack * PACK_FLOATS + lane;
                output_lane[lane] =
            ...
            output_hidden[s * HIDDEN_PACKS + h_pack] =
                pack_packed_float16(output_lane);
        }
    }
}

static void fp32_layernorm_and_route_full_model(
    float output_buf[SEQ_LEN][HIDDEN_SIZE],
    packed_float16_t *gamma,
    packed_float16_t *beta,
    bool final_layer,
    packed_float_stream_t &next_hidden_stream,
    packed_float16_t *final_output_hidden)
{
#pragma HLS INLINE off
    float gamma_cache[HIDDEN_SIZE];
    float beta_cache[HIDDEN_SIZE];
...
#pragma HLS UNROLL
    ...
    full_norm_reduce_pack:
        for (int h_pack = 0; h_pack < HIDDEN_PACKS; ++h_pack) {
#pragma HLS PIPELINE II=1
        full_norm_reduce_lane:
            for (int lane = 0; lane < PACK_FLOATS; ++lane) {
#pragma HLS UNROLL
                const float value =
                    output_buf[s][h_pack * PACK_FLOATS + lane];
    ...
    full_norm_write_pack:
        for (int h_pack = 0; h_pack < HIDDEN_PACKS; ++h_pack) {
#pragma HLS PIPELINE II=1
            float output_lane[PACK_FLOATS];
#pragma HLS ARRAY_PARTITION variable=output_lane complete dim=1
        full_norm_write_lane:
            for (int lane = 0; lane < PACK_FLOATS; ++lane) {
#pragma HLS UNROLL
                const int h = h_pack * PACK_FLOATS + lane;
                output_lane[lane] =
            ...
            const packed_float16_t word =
                pack_packed_float16(output_lane);
            if (final_layer) {
                final_output_hidden[s * HIDDEN_PACKS + h_pack] = word;
            } else {
                next_hidden_stream.write(word);
            }
        }
    }
}
...
#pragma HLS DATAFLOW
    ...
    v15_ffn_down_tilemajor_projection(gelu_stream, w2, output_buf);
}
...
void bert_ffn_down_residual_norm_fp32_kernel(
    packed_float_stream_t &gelu_stream,
    packed_float_stream_t &ffn_residual_stream,
    packed_float16_t *w2,
    packed_float16_t *b2,
    packed_float16_t *final_norm_gamma,
    packed_float16_t *final_norm_beta,
    packed_float16_t *output_hidden
) {
#pragma HLS INTERFACE axis port=gelu_stream
#pragma HLS INTERFACE axis port=ffn_residual_stream
#pragma HLS INTERFACE m_axi port=w2 offset=slave bundle=gmem_w2 depth=147456
#pragma HLS INTERFACE m_axi port=b2 offset=slave bundle=gmem_w2 depth=48
#pragma HLS INTERFACE m_axi port=final_norm_gamma offset=slave bundle=gmem_norm depth=48
#pragma HLS INTERFACE m_axi port=final_norm_beta offset=slave bundle=gmem_norm depth=48
#pragma HLS INTERFACE m_axi port=output_hidden offset=slave bundle=gmem_out depth=6144
#pragma HLS INTERFACE s_axilite port=w2 bundle=control
#pragma HLS INTERFACE s_axilite port=b2 bundle=control
#pragma HLS INTERFACE s_axilite port=final_norm_gamma bundle=control
#pragma HLS INTERFACE s_axilite port=final_norm_beta bundle=control
#pragma HLS INTERFACE s_axilite port=output_hidden bundle=control
#pragma HLS INTERFACE s_axilite port=return bundle=control
...
#pragma HLS ARRAY_PARTITION variable=output_buf cyclic factor=16 dim=2
    ...
    // Drain the residual before waiting for GELU.  If this order is reversed,
    // the residual FIFO can fill while FFN UP is still loading attn_mid_stream,
    // creating a circular backpressure deadlock.
    fp32_initialize_down_bias_residual(
        ffn_residual_stream, b2, output_buf);
    v15_ffn_down_tilemajor_projection(gelu_stream, w2, output_buf);
    fp32_layernorm_and_store(
        output_buf, final_norm_gamma, final_norm_beta, output_hidden);
}
...
full_ffn_up_layer:
    for (int layer = 0; layer < FULL_MODEL_LAYERS; ++layer) {
        v11_load_input_stream_matrix(attn_mid_stream, input_buf);
        v15_ffn_up_tilemajor_producer(
            input_buf,
            w1_all + layer * FFN_W1_WORDS_PER_LAYER,
            b1_all + layer * FFN_B1_WORDS_PER_LAYER,
            gelu_stream);
    }
}
...
#pragma HLS INTERFACE axis port=gelu_stream
#pragma HLS INTERFACE axis port=ffn_residual_stream
#pragma HLS INTERFACE axis port=next_hidden_stream
...
#pragma HLS INTERFACE m_axi port=final_norm_gamma_all offset=slave bundle=gmem_norm depth=576
#pragma HLS INTERFACE m_axi port=final_norm_beta_all offset=slave bundle=gmem_norm depth=576
#pragma HLS INTERFACE m_axi port=final_output_hidden offset=slave bundle=gmem_out depth=6144
...
#pragma HLS INTERFACE s_axilite port=final_output_hidden bundle=control
#pragma HLS INTERFACE s_axilite port=return bundle=control
...
full_ffn_down_layer:
    for (int layer = 0; layer < FULL_MODEL_LAYERS; ++layer) {
        fp32_initialize_down_bias_residual(
            ffn_residual_stream,
            b2_all + layer * FFN_HIDDEN_WORDS_PER_LAYER,
            output_buf);
        v15_ffn_down_tilemajor_projection(
            gelu_stream,
            w2_all + layer * FFN_W2_WORDS_PER_LAYER,
            output_buf);
        fp32_layernorm_and_route_full_model(
            output_buf,
            final_norm_gamma_all + layer * FFN_HIDDEN_WORDS_PER_LAYER,
            final_norm_beta_all + layer * FFN_HIDDEN_WORDS_PER_LAYER,
            layer == FULL_MODEL_LAYERS - 1,
            next_hidden_stream,
            final_output_hidden);
    }
}
