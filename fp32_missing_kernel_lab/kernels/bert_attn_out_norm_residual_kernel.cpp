...
#pragma HLS UNROLL
            ...
            // The normalized Attention output is both the FFN input and the
            // residual added after FFN DOWN.  Duplicate the packed word here
            // so SLR3 does not need another DDR read or a full residual buffer.
            ffn_residual_stream.write(word);
        }
    }
}

extern "C" void bert_attn_out_norm_residual_kernel(
    hidden_stream_t &context_stream,
    const bus_t *residual_in,
...
static void load_attention_residual_stream(
    hidden_stream_t &attention_residual_stream,
    float residual[SEQ_LEN][HIDDEN_SIZE])
{
#pragma HLS INLINE off
FULL_OUT_LOAD_RESIDUAL_ROW:
    for (int s = 0; s < SEQ_LEN; ++s) {
    FULL_OUT_LOAD_RESIDUAL_PACK:
        for (int p = 0; p < PACKS; ++p) {
#pragma HLS PIPELINE II=1
            float lane[PACK_SIZE];
#pragma HLS ARRAY_PARTITION variable=lane complete dim=1
            unpack_bus16(attention_residual_stream.read(), lane);
        FULL_OUT_LOAD_RESIDUAL_LANE:
            for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
                residual[s][p * PACK_SIZE + i] = lane[i];
            }
        }
    }
}

static void residual_norm_to_full_model_streams(
    float residual[SEQ_LEN][HIDDEN_SIZE],
    float projected[SEQ_LEN][HIDDEN_SIZE],
    const bus_t *gamma,
    const bus_t *beta,
...
#pragma HLS ARRAY_PARTITION variable=sum_bank complete dim=1
#pragma HLS ARRAY_PARTITION variable=square_bank complete dim=1
    FULL_OUT_NORM_BANK_INIT:
        for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
            sum_bank[i] = 0.0f;
            square_bank[i] = 0.0f;
        }
    FULL_OUT_NORM_REDUCE_PACK:
        for (int p = 0; p < PACKS; ++p) {
#pragma HLS PIPELINE II=1
        FULL_OUT_NORM_REDUCE_LANE:
            for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
                const int h = p * PACK_SIZE + i;
                const float x = residual[s][h] + projected[s][h];
    ...
    FULL_OUT_NORM_WRITE_PACK:
        for (int p = 0; p < PACKS; ++p) {
#pragma HLS PIPELINE II=1
            float lane[PACK_SIZE];
#pragma HLS ARRAY_PARTITION variable=lane complete dim=1
        FULL_OUT_NORM_WRITE_LANE:
            for (int i = 0; i < PACK_SIZE; ++i) {
#pragma HLS UNROLL
                const int h = p * PACK_SIZE + i;
                const float x = residual[s][h] + projected[s][h];
                lane[i] =
                    (x - mean) * inv_std * gamma_cache[h] + beta_cache[h];
            }
            const bus_t word = pack_bus16(lane);
            attn_mid_stream.write(word);
            ffn_residual_stream.write(word);
...
extern "C" void bert_attn_out_norm_12layer_kernel(
    hidden_stream_t &context_stream,
    hidden_stream_t &attention_residual_stream,
    const bus_t *attn_o_w_all,
    const bus_t *attn_o_b_all,
...
FULL_OUT_LAYER:
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        ...
        const int norm_offset = layer * PACKS;

        load_attention_residual_stream(
            attention_residual_stream, residual);
        initialize_projection(attn_o_b_all + bias_offset, projected);
    FULL_OUT_CONTEXT_GROUP:
        for (int group = 0; group < HEAD_GROUPS; ++group) {
            read_context_group(context_stream, context_group);
            accumulate_projection_group(
                context_group, attn_o_w_all + weight_offset,
                group, projected);
        }
        residual_norm_to_full_model_streams(
            residual, projected,
            attn_norm_gamma_all + norm_offset,
            attn_norm_beta_all + norm_offset,
