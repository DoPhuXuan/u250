...
def find_reports(root):
    rows = []
    standalone_rows = []
    for standalone_rpt in sorted(root.glob("csynth*.rpt")):
        standalone_rows.append(parse_rpt(standalone_rpt, root))

    project_rows = []
    for project in sorted(root.glob("proj_*")):
...
def main():
    ...
    csv_path = Path(sys.argv[2]) if len(sys.argv) > 2 else root / "ffn_kernel_report_summary.csv"
